 ...
 * is covered by lib/abc/classify.test.ts; what is here is everything that only
...
test('answers a French browser in French, on both tools', async ({ browser }) => {
  ...
  for (const route of ['/abc', '/']) {
    ...
    await expect(page.getByLabel('Devise'), route).toHaveValue('MAD');
    await page.close();
  }
...
test('reaches the sibling tool, and is reached from it', async ({ page }) => {
  await page.goto(PAGE);
  await ready(page);

  await page.getByRole('link', { name: 'Inventory ordering' }).click();
  await expect(page.getByRole('link', { name: 'ABC analysis' })).toBeVisible();
});
